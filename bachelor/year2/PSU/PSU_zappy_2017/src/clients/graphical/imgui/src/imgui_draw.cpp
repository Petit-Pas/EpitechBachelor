...
// Render-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level src::PushClipRect() to affect logic (hit-testing and widget culling)
...
void ImDrawList::AddText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
    ...
    IM_ASSERT(font->ContainerAtlas->TexID == _TextureIdStack.back());  // Use high-level src::PushFont() or low-level ImDrawList::PushTextureId() to change font.
...
// Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scale than src expects, or if there is a difference between your window resolution and framebuffer resolution.
...
// The white texels on the top left are the ones we'll use everywhere in src to render filled shapes.
...
ImFont* ImFontAtlas::AddFont(const ImFontConfig* font_cfg)
{
    ...
    if (!font_cfg->MergeMode)
        ...
        IM_ASSERT(!Fonts.empty()); // When using MergeMode make sure that a font has already been added before. You can use src::GetIO().Fonts->AddFontDefault() to add the default imgui font.
...
{
    if (!text_end)
        text_end = text_begin + strlen(text_begin); // src functions generally already provides a valid text_end, so this is merely to handle direct calls.
