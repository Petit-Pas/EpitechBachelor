...
// Call and read src::ShowDemoWindow() in imgui_demo.cpp for demo code.
// Newcomers, read 'Programmer guide' below for notes on how to setup Dear src in your codebase.
...
/*
 ...
 - PROGRAMMER GUIDE (read me!)
   - Read first
   - How to update to a newer version of Dear src
   - Getting started with integrating Dear src in your code/engine
 - API BREAKING CHANGES (read me when you update!)
 - ISSUES & TODO LIST
 - FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
   - How can I help?
   - What is ImTextureID and how do I display an image?
   - I integrated Dear src in my engine and the text or lines are blurry..
   - I integrated Dear src in my engine and some elements are clipping or disappearing when I move windows around..
   - How can I have multiple widgets with the same label? Can I have widget without a label? (Yes). A primer on labels/IDs.
   - How can I tell when Dear src wants my mouse/keyboard inputs VS when I can pass them to my application?
   - How can I load a different font than the default?
   - How can I easily use icons in my application?
   - How can I load multiple fonts?
   - How can I display and input non-latin characters such as Chinese, Japanese, Korean, Cyrillic?
   - How can I preserve my Dear src context across reloading a DLL? (loss of the global/static variables)
   - How can I use the drawing facilities without an src window? (using ImDrawList API)
 - ISSUES & TODO-LIST
 - CODE
 ...
 - Click upper right corner to close a window, available when 'bool* p_open' is passed to src::Begin().
 ...
 - Call and read src::ShowDemoWindow() for demo code demonstrating most features.
 - You can learn about immediate-mode gui principles at http://www.johno.se/book/imgui.hpptml or watch http://mollyrocket.com/861

 HOW TO UPDATE TO A NEWER VERSION OF DEAR IMGUI

 - Overwrite all the sources files except for imconfig.hpp(if you have made modification to your copy of imconfig.h)
 ...
 - Add the Dear src source files to your projects, using your preferred build system.
   It is recommended you build the .cpp files as part of your project and not as a library.
 - You can later customize the imconfig.hppfile to tweak some compilation time behavior, such as integrating imgui types with your own maths types.
 - See examples/ folder for standalone sample applications.
 - You may be able to grab and copy a ready made imgui_impl_*** file from the examples/.
 - When using Dear src, your programming IDE is your friend: follow the declaration of variables, functions and types to find comments about them.

 - Init: retrieve the ImGuiIO structure with src::GetIO() and fill the fields marked 'Settings': at minimum you need to set io.DisplaySize
 ...
 - Every frame:
    ...
    - Call src::NewFrame() to begin the frame
    - You can use any src function you want between NewFrame() and Render()
    - Call src::Render() as late as you can to end the frame and finalize render data. it will call your io.RenderDrawListFn handler.
 ...
 - All rendering information are stored into command-lists until src::Render() is called.
 - Dear src never touches or knows about your GPU state. the only function that knows about GPU is the RenderDrawListFn handler that you provide.
 ...
 - A minimal application skeleton may be:
     ...
     while (true)
     {
        ...
        // Call NewFrame(), after this point you can use src::* functions anytime
        src::NewFrame();

        // Most of your application code here
        MyGameUpdate(); // may use any src functions, e.g. src::Begin("My window"); src::Text("Hello, world!"); src::End();
        MyGameRender(); // may use any src functions as well!
 ...
 - A minimal render function skeleton may be:
    ...
    void void MyRenderFunction(ImDrawData* draw_data)(ImDrawData* draw_data)
    {
       ...
       {
          const ImDrawVert* vtx_buffer = cmd_list->VtxBuffer.Data;  // vertex buffer generated by src
          const ImDrawIdx* idx_buffer = cmd_list->IdxBuffer.Data;   // index buffer generated by src
 ...
 - When calling NewFrame(), the 'io.WantCaptureMouse'/'io.WantCaptureKeyboard'/'io.WantTextInput' flags are updated. 
   They tell you if src intends to use your inputs. So for example, if 'io.WantCaptureMouse' is set you would typically want to hide
 ...
 - 2017/11/18 (1.53) - Style, Begin: removed ImGuiWindowFlags_ShowBorders window flag. Borders are now fully set up in the ImGuiStyle structure (see e.g. style.FrameBorderSize, style.WindowBorderSize). Use src::ShowStyleEditor() to look them up.
 ...
 - 2016/04/26 (1.49) - changed ImDrawList::PushClipRect(ImVec4 rect) to ImDraw::PushClipRect(Imvec2 min,ImVec2 max,bool intersect_with_current_clip_rect=false). Note that higher-level src::PushClipRect() is preferable because it will clip at logic/widget level, whereas ImDrawList::PushClipRect() only affect your renderer.
 ...
 - 2015/05/11 (1.40) - changed BeginPopup() API, takes a string identifier instead of a bool. src needs to manage the open/closed state of popups. Call OpenPopup() to actually set the "open" state of a popup. BeginPopup() returns true if the popup is opened.
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       this sequence:
                           ...
                           src::GetDefaultFontData(NULL, NULL, &png_data, &png_size);
                           // <Copy to GPU>
                       became:
                           unsigned char* pixels;
                           int width, height;
              ...
              (1.30) - removed src::IsItemFocused() in favor of src::IsItemActive() which handles all widgets
 ...
 Q: How can I help?
 A: - If you are experienced enough with Dear src and with C/C++, look at the todo list and see how you want/can help!
 ...
 A: ImTextureID is a void* used to pass renderer-agnostic texture references around until it hits your render function.
    Dear src knows nothing about what those bits represent, it just passes them around. It is up to you to decide what you want the void* to carry!
    It could be an identifier to your OpenGL texture (cast GLuint to void*), a pointer to your custom engine material (cast MyMaterial* to void*), etc.
    At the end of the chain, your renderer takes this void* to cast it back into whatever it needs to select a current texture to render.
    Refer to examples applications, where each renderer (in a imgui_impl_xxxx.cpp file) is treating ImTextureID as a different thing.
    (c++ tip: OpenGL uses integers to identify textures. You can safely store an integer into a void*, just cast it to void*, don't take it's address!)
    To display a custom image/texture within an src window, you may use src::Image(), src::ImageButton(), ImDrawList::AddImage() functions.
    Dear src will generate the geometry and draw calls using the ImTextureID that you passed and which your renderer can use.
    It is your responsibility to get textures uploaded to your GPU.

 Q: I integrated Dear src in my engine and the text or lines are blurry..
 A: In your Render function, try translating your projection matrix by (0.5f,0.5f) or (0.375f,0.375f).
    Also make sure your orthographic projection matrix and io.DisplaySize matches your actual framebuffer dimension.

 Q: I integrated Dear src in my engine and some elements are clipping or disappearing when I move windows around..
 A: You are probably mishandling the clipping rectangles in your render function. 
    Rectangles provided by src are defined as (x1=left,y1=top,x2=right,y2=bottom) and NOT as (x1,y1,width,height).

 Q: Can I have multiple widgets with the same label? Can I have widget without a label?
 A: Yes. A primer on the use of labels/IDs in Dear src..

   - Elements that are not clickable, such as Text() items don't need an ID.

   - Interactive widgets require state to be carried over multiple frames (most typically Dear src often needs to remember what is
 ...
 Q: How can I tell when Dear src wants my mouse/keyboard inputs VS when I can pass them to my application?
 A: You can read the 'io.WantCaptureMouse'/'io.WantCaptureKeyboard'/'ioWantTextInput' flags from the ImGuiIO structure. 
    ...
    Preferably read the flags after calling src::NewFrame() to avoid them lagging by one frame. But reading those flags before calling NewFrame() is
    also generally ok, as the bool toggles fairly rarely and you don't generally expect to interact with either Dear src or your application during
    the same frame when that transition occurs. Dear src is tracking dragging and widget activity that may occur outside the boundary of a window,
    ...
    (Advanced note: text input releases focus on Return 'KeyDown', so the following Return 'KeyUp' event that your application receive will typically 
     ...
     were for Dear src, e.g. with an array of bool, and filter out the corresponding key-ups.)
 ...
 A: Use the font atlas to pack them into a single texture:
    (Read extra_fonts/README.txt and the code in ImFontAtlas for more details.)
      ...
      // use src::PushFont()/src::PopFont() to change the font at runtime
 ...
 Q: How can I preserve my Dear src context across reloading a DLL? (loss of the global/static variables)
 ...
 Q: How can I use the drawing facilities without an src window? (using ImDrawList API)
 A: The easiest way is to create a dummy window. Call Begin() with NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flag, 
    ...
    You can also perfectly create a standalone ImDrawList instance _but_ you need src to be initialized because ImDrawList pulls from src
 ...
 - tip: call and read the ShowDemoWindow() code in imgui_demo.cpp for more example of how to use src!
...
#pragma clang diagnostic ignored "-Wexit-time-destructors"  // warning : declaration requires an exit-time destructor       // exit-time destruction order is undefined. if MemFree() leads to users code that has been disabled before exit it might cause problems. src coding style welcomes static/globals.
...
// Implicitely used by all src functions. Always assumed to be != NULL. Change to a different context by calling src::SetCurrentContext()
// If you are hot-reloading this code in a DLL you will lose the static/global variables. Create your own context+font atlas instead of relying on those default (see FAQ entry "How can I preserve my src context across reloading a DLL?").
// src is currently not thread-safe because of this variable. If you want thread-safety to allow N threads to access N different contexts, you might work around it by:
// - Having multiple instances of the src code compiled inside different namespace (easiest/safest, if you have a finite number of contexts)
// - or: Changing this variable to be TLS. You may #define GImGui in imconfig.hppfor further custom hackery. Future development aim to make this context pointer explicit to all calls. Also read https://github.com/ocornut/imgui/issues/586
#ifndef GImGui
...
{
    Alpha                   = 1.0f;             // Global alpha applies to everything in src
...
// Memory allocated with src::MemAlloc(), must be freed by user using src::MemFree()
...
void ImGuiListClipper::End()
{
    if (ItemsCount < 0)
        return;
    // In theory here we should assert that src::GetCursorPosY() == StartPosY + DisplayEnd * ItemsHeight, but it feels saner to just seek at the end and not assert/crash the user.
...
//-----------------------------------------------------------------------------
// Internal API exposed in imgui_internal.hpp
//-----------------------------------------------------------------------------
...
// Internal state access - if you want to share src state between modules (e.g. DLL) or allocate it yourself
...
void ImGui::NewFrame()
{
    ...
    // If mouse was first clicked outside of src bounds we also cancel out hovering.
...
void ImGui::Shutdown()
{
    ...
    // Cleanup of other data are conditional on actually having initialize src.
    if (!g.Initialized)
        return;
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_render_list, ImDrawList* draw_list)
{
    ...
    // B) If you need/want meshes with more than 64K vertices, uncomment the '#define ImDrawIdx unsigned int' line in imconfig.hppto set the index size to 4 bytes.
...
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);                       // Forgot to call src::NewFrame()
...
// Internal src functions to render text
...
// Push a new src window to add widgets to.
// - A default window called "Debug" is automatically stacked at the beginning of every frame so you can use widgets without explicitly calling a Begin/End pair.
// - Begin/End can be called multiple times during the frame with the same window name to append content.
// - The window name is used as a unique identifier to preserve window information across frames (and save rudimentary information to the .ini file).
//   You can use the "##" or "###" markers to use the same label with different id, or same id with different label. See documentation at the top of this file.
// - Return false when window is collapsed, so you can early out in your code. You always need to call src::End() even if false is returned.
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    IM_ASSERT(g.Initialized);                       // Forgot to call src::NewFrame()
    IM_ASSERT(g.FrameCountEnded != g.FrameCount);   // Called src::Render() or src::EndFrame() and haven't called src::NewFrame() again yet
...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
    ...
    case ImGuiCol_PlotLines: return "PlotLine";
...
// Tip: use src::PushID()/PopID() to push indices or pointers in the ID stack.
...
// Start logging src output to TTY
...
// Start logging src output to given file
...
// Start logging src output to clipboard
...
// Wrapper for stb_textedit.hpp to edit text (our wrapper is for: statically sized buffer, single-line, wchar characters. InputText converts between UTF-8 and wchar)
namespace ImGuiStb
{
...
#endif
#define STB_TEXTEDIT_MOVEWORDLEFT   STB_TEXTEDIT_MOVEWORDLEFT_IMPL    // They need to be #define for stb_textedit.hpp
...
// We don't use an enum so we can build even with conflicting symbols (if another user of stb_textedit.hpp leak their STB_TEXTEDIT_K_* symbols)
...
// Local src-only clipboard implementation, if user hasn't defined better clipboard handlers
static const char* GetClipboardTextFn_DefaultImpl(void*)
{
    ImGuiContext& g = *GImGui;
...
{
    if (ImGui::Begin("src Metrics", p_open))
    {
        ImGui::Text("Dear src %s", ImGui::GetVersion());
