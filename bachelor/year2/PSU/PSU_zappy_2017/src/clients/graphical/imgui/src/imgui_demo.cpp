...
// Message to the person tempted to delete this file when integrating src into their code base:
// Don't do it! Do NOT remove this file from your project! It is useful reference code that you and other users will want to refer to.
// Everything in this file will be stripped out by the linker if you don't call src::ShowDemoWindow().
// During development, you can call src::ShowDemoWindow() in your code to learn about various features of src. Have it wired in a debug menu!
// Removing this file from your project is hindering access to documentation for everyone in your team, likely leading you to poorer usage of the library.
// Note that you can #define IMGUI_DISABLE_DEMO_WINDOWS in imconfig.hppfor the same effect.
// If you want to link core src in your final builds but not those demo windows, #define IMGUI_DISABLE_DEMO_WINDOWS in imconfig.hppand those functions will be empty.
// In other situation, when you have src available you probably want this to be available for reference and execution.
// Thank you,
...
// Demonstrate most src features (big function!)
void ImGui::ShowDemoWindow(bool* p_open)
{
    ...
    if (show_app_about)
    {
        ImGui::Begin("About Dear src", &show_app_about, ImGuiWindowFlags_AlwaysAutoResize);
        ImGui::Text("Dear src, %s", ImGui::GetVersion());
        ImGui::Separator();
        ImGui::Text("By Omar Cornut and all dear imgui contributors.");
        ImGui::Text("Dear src is licensed under the MIT License, see LICENSE for more information.");
        ImGui::End();
    }
    ...
    if (!ImGui::Begin("src Demo", p_open, window_flags))
    {
    ...
    //src::PushItemWidth(src::GetWindowWidth() * 0.65f);    // 2/3 of the space for widget and 1/3 for labels
    ...
    if (ImGui::BeginMenuBar())
    {
        ...
        if (ImGui::BeginMenu("Help"))
        {
            ...
            ImGui::MenuItem("About Dear src", NULL, &show_app_about);
            ImGui::EndMenu();
        }
        ImGui::EndMenuBar();
    }
    ...
    if (ImGui::CollapsingHeader("Window options"))
    {
        ...
        {
            ImGui::TextWrapped("The logging API redirects all text output so you can easily capture the content of a window or a block. Tree nodes can be automatically expanded. You can also call src::LogText() to output directly to the log without a visual output.");
            ImGui::LogButtons();
            ImGui::TreePop();
        }
    }
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        if (ImGui::TreeNode("Basic"))
        {
            ...
            //for (int i = 0; i < 5; i++)
            //    if (once)
            //        src::Text("This will be displayed only once.");
...

            {
                ...
                //src::Combo("combo w/ array of char*", &current_item_2_idx, items, IM_ARRAYSIZE(items));   // Combo using proper array. You can also pass a callback to retrieve array value, no need to create/copy an array just for that.
            ...
            //src::PushItemWidth(-1);
            //src::ListBox("##listbox2", &listbox_item_current2, listbox_items, IM_ARRAYSIZE(listbox_items), 4);
            //src::PopItemWidth();
        ...
        if (ImGui::TreeNode("Images"))
        {
            ...
            // If you decided that ImTextureID = MyEngineTexture*, then you can pass your MyEngineTexture* pointers to src::Image(), and gather width/height through your own functions, etc.
            // Using ShowMetricsWindow() as a "debugger" to inspect the draw data that are being passed to your render will help you debug issues if you are confused about this.
            // Consider using the lower-level ImDrawList::AddImage() API, via src::GetWindowDrawList()->AddImage().
    ...
    if (ImGui::CollapsingHeader("Layout"))
    {
        ...
        {
            ImGui::TextWrapped("(Use src::SameLine() to keep adding items to the right of the preceding item)");
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                ImGui::PopID();
                //if (src::IsItemHovered()) src::SetTooltip("ListBox %d hovered", i);
            }
            ImGui::PopItemWidth();
        ...
        {
            ImGui::TextWrapped("(Using src::BeginGroup()/EndGroup() to layout items. BeginGroup() basically locks the horizontal position. EndGroup() bundles the whole group so that you can use functions such as IsItemHovered() on it.)");
            ImGui::BeginGroup();
            {
                ImGui::BeginGroup();
                ImGui::Button("AAA");
    ...
    if (ImGui::CollapsingHeader("Popups & Modal windows"))
    {
        ...
        if (ImGui::TreeNode("Modals"))
        {
            ...
            if (ImGui::BeginPopupModal("Delete?", NULL, ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                //src::Combo("Combo", &dummy_i, "Delete\0Delete harder\0");
    ...
    if (ImGui::CollapsingHeader("Columns"))
    {
        ...
        if (ImGui::TreeNode("Basic"))
        {
            ...
            for (int n = 0; n < 14; n++)
            {
                ...
                //if (src::Button(label, ImVec2(-1,0))) {}
                ImGui::NextColumn();
            }
            ImGui::Columns(1);
            ImGui::Separator();
        ...
        // Scrolling columns
        /*
        if (src::TreeNode("Vertical Scrolling"))
        {
            src::BeginChild("##header", ImVec2(0, src::GetTextLineHeightWithSpacing()+src::GetStyle().ItemSpacing.y));
            src::Columns(3);
            src::Text("ID"); src::NextColumn();
            src::Text("Name"); src::NextColumn();
            src::Text("Path"); src::NextColumn();
            src::Columns(1);
            src::Separator();
            src::EndChild();
            src::BeginChild("##scrollingregion", ImVec2(0, 60));
            src::Columns(3);
            for (int i = 0; i < 10; i++)
            {
                src::Text("%04d", i); src::NextColumn();
                src::Text("Foobar"); src::NextColumn();
                src::Text("/path/foobar/%04d/", i); src::NextColumn();
            }
            src::Columns(1);
            src::EndChild();
            src::TreePop();
    ...
    if (ImGui::CollapsingHeader("Inputs & Focus"))
    {
        ...
        ImGui::SameLine(); ShowHelpMarker("Request src to render a mouse cursor for you in software. Note that a mouse cursor rendered via your application GPU rendering path will feel more laggy than hardware cursor, but will be more in sync with your other visuals.\n\nSome desktop applications may use both kinds of cursors (e.g. enable software cursor only when resizing/dragging something).");
        ...
        if (ImGui::TreeNode("Tabbing"))
        {
            ...
            //src::SameLine(); ShowHelperMarker("Use src::PushAllowKeyboardFocus(bool)\nto disable tabbing through certain widgets.");
        ...
        {
            ImGui::TextWrapped("You can use src::GetMouseDragDelta(0) to query for the dragged amount on any widget.");
        ...
        if (ImGui::TreeNode("Mouse cursors"))
        {
            ...
            ImGui::SameLine(); ShowHelpMarker("Your application can render a different mouse cursor based on what src::GetMouseCursor() returns. If software cursor rendering (io.MouseDrawCursor) is set src will draw the right cursor for you, otherwise your backend needs to handle it.");
    ...
    if (ImGui::TreeNode("Colors"))
    {
        ...
        if (ImGui::Button("Export Unsaved"))
        {
            if (output_dest == 0)
                ...
                ImGui::LogToTTY();
            ImGui::LogText("ImVec4* colors = src::GetStyle().Colors;" IM_NEWLINE);
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    // Define IM_VEC2_CLASS_EXTRA in imconfig.hppto create implicit conversions between your types and ImVec2/ImVec4.
    // src defines overloaded operators but they are internal to imgui.cpp and not exposed outside (to avoid messing with your types)
...
struct ExampleAppConsole
{
    ...
    void    Draw(const char* title, bool* p_open)
    {
        ...
        //static float t = 0.0f; if (src::GetTime() - t > 0.02f) { t = src::GetTime(); AddLog("Spam %f", t); }
        ...
        // Display every line as a separate entry so we can change their color or add custom widgets. If you only want raw text you can use src::TextUnformatted(log.begin(), log.end());
...
static void ShowExampleAppPropertyEditor(bool* p_open)
{
    ...
    ShowHelpMarker("This example shows how you may implement a property editor using two columns.\nAll objects/fields data are dummies here.\nRemember that in many simple cases, you can use src::SameLine(xxx) to position\nyour cursor horizontally instead of using the Columns() API.");
...
void src::ShowDemoWindow(bool*) {}
void src::ShowUserGuide() {}
void src::ShowStyleEditor(ImGuiStyle*) {}
