...
#ifndef CPP_ARCADE_CACA_HPP
#define CPP_ARCADE_CACA_HPP
...
namespace caca {
	...
	public:
		...
		void renderCanvas(Canvas *, uint8_t *, unsigned int, unsigned int, unsigned int
		);
	...
	class Dither {
	public:
		Dither(unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int,
			unsigned int
		);
		~Dither();

		void setPalette(uint32_t r[], uint32_t g[], uint32_t b[], uint32_t a[]
		);
